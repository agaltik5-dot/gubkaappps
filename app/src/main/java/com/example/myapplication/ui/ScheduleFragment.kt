...
class ScheduleFragment : Fragment(R.layout.fragment_schedule) {
    ...
    private fun updateUIForPosition(position: Int) {
        ...
        // 2. Обновляем числа в кнопках (ПН-ВС) для текущей недели
        ...
        dayViews.forEachIndexed { index, dayView ->
            if (dayView != null) {
                ...
                // Если этот день совпадает с выбранной позицией — выделяем его
                if (index == getDayIndexForCalendar(date)) {
                    selectDayVisuals(dayView)
                }
    ...
    // Возвращает 0 для ПН, 1 для ВТ ... 6 для ВС
    ...
    private fun setupDaySelector(view: View) {
        ...
        dayViews.forEachIndexed { index, dayView ->
            dayView?.setOnClickListener {
                ...
                // Вычисляем смещение до нужного дня в текущей неделе
    ...
    private fun selectDayVisuals(view: View) {
        ...
        if (selectedDayView == view) return
