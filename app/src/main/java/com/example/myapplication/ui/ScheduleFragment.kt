...
class ScheduleFragment : Fragment(R.layout.fragment_schedule) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view.findViewById<View>(R.id.btn_select_group_shortcut)?.setOnClickListener {
            ...
            mainActivity?.switchToTab(3) {
                val profileFrag = mainActivity.supportFragmentManager.findFragmentById(R.id.fragment_container) as? com.example.myapplication.ui.ProfileFragment
                profileFrag?.openStudentData()
                
                // Wait for ProfileFragment to finish transaction
                view.postDelayed({
                    val studentDataFrag = mainActivity.supportFragmentManager.findFragmentById(R.id.fragment_container) as? com.example.myapplication.ui.StudentDataFragment
                    studentDataFrag?.openEditProfile()
                }, 100)
            }
        }
    ...
    private fun toggleHistoryExpansion() {
        ...
        // Сначала подготавливаем UI (если нужно открыть), чтобы избежать мерцания
        if (isExpanded) {
            updateRecentListUI()
        }
        
        // Анимируем расширение (250мс)
        val transition = AutoTransition().setDuration(250)
        view?.let { TransitionManager.beginDelayedTransition(it as ViewGroup, transition) }
    ...
    private fun setupViewPagers(view: View) {
        ...
        viewPager?.registerOnPageChangeCallback(object : ViewPager2.OnPageChangeCallback() {
            override fun onPageSelected(position: Int) {
                ...
                if (!isSyncingFromTop) {
                    // Авто-раскрытие хедера на пустых днях
                    val lessons = scheduleCache.getOrPut(position) {
                        scheduleRepository.getScheduleForDate(currentGroupId, selectedDate)
                    }
                    if (lessons.isEmpty()) {
                        view?.findViewById<AppBarLayout>(R.id.app_bar)?.setExpanded(true, true)
                    }
